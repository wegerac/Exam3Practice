...
def run_test_integers():
    ...
    # Test 2
    exp = [1, 2, 3, 4, 5, 6, 7, 8, 7, 6, 5, 4, 3, 2, 1]
    act = integers([[1, 2, 3, 'hello there', 4, 5, 6, 7, 8,
                     'lovely weather we are having, yes?', 7, 6, 5, 4, 3],
                    [],
                    [2],
                    [1, 'end test']])
        ...
        if type(sequence_of_sequences[k]) is str:
            for i in range(len(sequence_of_sequences[k])):
                if type(sequence_of_sequences[k][i]) is str:
                    for j in range(len(sequence_of_sequences[k][i])):
                        if sequence_of_sequences[k][i][j].isupper():
                            string = string + sequence_of_sequences[k][i][j]
